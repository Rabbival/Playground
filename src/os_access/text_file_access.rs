...
pub fn create_file(folder_to_create_in: FolderToAccess, file_name: &String) -> std::io::Result<()> {
    ...
    let file_path =
        PathBuf::from(&folder_to_create_in.to_string()).join(&full_file_name.name_with_postfix);
...
pub fn write_to_file(
    ...
    let full_file_name = SystemFileName::from_name(file_name, SystemFileType::TextFile);
    let file_path =
        PathBuf::from(&folder_to_put_file_in.to_string()).join(&full_file_name.name_with_postfix);
...
pub fn append_to_file(
    ...
    let file_path = PathBuf::from(&folder_where_the_file_is.to_string())
        .join(&full_file_name.name_with_postfix);
...
pub fn delete_text_file(
    ...
    let full_file_name = SystemFileName::from_name(file_name, SystemFileType::TextFile);
    let file_path =
        PathBuf::from(&folder_to_delete_from.to_string()).join(&full_file_name.name_with_postfix);
